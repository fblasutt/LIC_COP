...
class HouseholdModelClass(EconModelClass):
        ...
        for iL,love in enumerate(par.grid_love):
            for iA,A in enumerate(par.grid_A):
                ...
                for iP,power in enumerate(par.grid_power):
                    ...
                    skip=True if (t==par.T-1) else False
                    remain_Cw_priv[iP], remain_Cm_priv[iP], remain_C_pub[iP], remain_Vw[iP], remain_Vm[iP] = \
                        solve_remain_couple2(skip,t,M_resources,iL,iP,power,Vw_next,Vm_next,
                                              sol.pre_Ctot_Cw_priv,sol.pre_Ctot_Cm_priv,sol.Vw_plus_vec,sol.Vm_plus_vec,
                                              ...
                                              par.T,par.grid_shock_love,par.grid_A,par.grid_weight_love,par.beta,par.num_shock_love,starting_val=starting_val)
    ...
    def solve_remain_couple(self,t,M_resources,iL,iP,power,Vw_next,Vm_next,starting_val = None):
        ...
        args=(t,M_resources,iL,power,Vw_next,Vm_next,
        ...
        if t==(par.T-1): # Terminal period
            ...
            Vw = usr.util_j(Cw_priv,C_pub,woman,par.rho_w,par.phi_w,par.alpha1_w,par.alpha2_w,par.grid_love[iL])
            Vm = usr.util_j(Cm_priv,C_pub,man  ,par.rho_m,par.phi_m,par.alpha1_m,par.alpha2_m,par.grid_love[iL])
            ...
            # def obj(x):
            #     return -value_of_choice_couple2(x,t,M_resources,iL,power,Vw_next,Vm_next,
            #                             sol.pre_Ctot_Cw_priv[iP],sol.pre_Ctot_Cm_priv[iP],sol.Vw_plus_vec,sol.Vm_plus_vec,
            #                             par.grid_love,par.num_Ctot,par.grid_Ctot,
            #                             par.rho_w,par.phi_w,par.alpha1_w,par.alpha2_w,
            #                             par.rho_m,par.phi_m,par.alpha1_m,par.alpha2_m,
            #                             par.T,par.grid_shock_love,par.grid_A,par.grid_weight_love,par.beta,par.num_shock_love)[0]

            
            
            C_tot=golden_section_search.optimizer(obj,1.0e-6, M_resources - 1.0e-6,args=args)
            
            # res = optimize.minimize(obj,x0[0],bounds=((1.0e-6, M_resources - 1.0e-6),) ,method='SLSQP',args=args) 
            # C_tot = res.x[0]
        # implied consumption allocation (re-calculation)
            _, Cw_priv, Cm_priv, C_pub, Vw,Vm = value_of_choice_couple3(C_tot,*args)
...
@njit           
def value_of_choice_couple3(Ctot,t,M_resources,iL,power,Vw_next,Vm_next,
                            pre_Ctot_Cw_priviP,pre_Ctot_Cm_priviP,Vw_plus_vec,Vm_plus_vec,
                            grid_love,num_Ctot,grid_Ctot,
                            rho_w,phi_w,alpha1_w,alpha2_w,
                            rho_m,phi_m,alpha1_m,alpha2_m,
...
@njit
def solve_remain_couple2(skip,t,M_resources,iL,iP,power,Vw_next,Vm_next,
                            pre_Ctot_Cw_priv,pre_Ctot_Cm_priv,Vw_plus_vec,Vm_plus_vec,
                            grid_love,num_Ctot,grid_Ctot,
                            rho_w,phi_w,alpha1_w,alpha2_w,
                            rho_m,phi_m,alpha1_m,alpha2_m,
                            T,grid_shock_love,grid_A,grid_weight_love,beta,num_shock_love,starting_val = None):

    
  
    if (Vw_next is None) or (Vm_next is None): # Terminal period
        C_tot = M_resources
        
        # current utility from consumption allocation
        
        Cw_priv, Cm_priv, C_pub = intraperiod_allocationj(C_tot,num_Ctot,grid_Ctot,pre_Ctot_Cw_priv[iP],pre_Ctot_Cm_priv[iP])
        Vw = usr.util_j(Cw_priv,C_pub,woman,rho_w,phi_w,alpha1_w,alpha2_w,grid_love[iL])
        Vm = usr.util_j(Cm_priv,C_pub,man  ,rho_m,phi_m,alpha1_m,alpha2_m,grid_love[iL])

    else:
          
        args=(t,M_resources,iL,power,Vw_next,Vm_next,
                              pre_Ctot_Cw_priv[iP],pre_Ctot_Cm_priv[iP],Vw_plus_vec,Vm_plus_vec,
                              grid_love,num_Ctot,grid_Ctot,
                              rho_w,phi_w,alpha1_w,alpha2_w,
                              rho_m,phi_m,alpha1_m,alpha2_m,
                              T,grid_shock_love,grid_A,grid_weight_love,beta,num_shock_love)
